...
def extract_beam(beam_input):
    ...
    profile_shape = "test"
    #profile_shape = psets_props['Other']['Family Name'] #comment out wether pset is defined in this way or not
...
def extract_wall(wall_input):
    ...
    #if height < 0.5 or thickness < 0.5:
    #    None
    #else:
...
def extract_window_door(windoor_input):
    wall = windoor_input
    wall_type = ifcopenshell.util.element.get_type(wall)
    ...
    id_windoor = windoor_input.is_a()
    
    wall_dict = {" Guid ": element_guid , " Element ": id_windoor , " Material ": material_name , "Quality": quality, "Material label": quality_short, " Length [ m ]": thickness," Height [ m ]": height , " Width [ m ]": length , "Volume [m3]" : volume, "Area [m2]": area}
    #print(wall_input,id, x,y,z, material)
...
i_beam = i_col = i_slab = i_wall = i_wall_standardcase = i_wall_type = i_window = i_door = 0

ifc_file = ifcopenshell.open(r'C:\Users\e_ski\Downloads\RIB_Drivhuset_NYBYGG.ifc')
...
custom_order = ["IfcBeam", "IfcColumn", "IfcSlab", "IfcWall","IfcWindow", "IfcDoor"]

# Convert 'Element' column to a categorical type with the custom order
ext_elements_df[" Element "] = pd.Categorical(ext_elements_df[" Element "], categories=custom_order, ordered=True)

# Sort the DataFrame based on the custom order
ext_elements_df_sort = ext_elements_df.sort_values(" Element ")

# Add an index column
ext_elements_df_sort.insert(0, 'Index', range(0, len(ext_elements_df_sort)))

# Save the sorted DataFrame to an Excel file
ext_elements_df_sort.to_excel(r'C:\Users\e_ski\OneDrive\Documents\NTNU\10. semester\Masteroppgave\Study case Tennebekk\test multi bygg.xlsx', index=False)
